...
{
	MyString change;
	char inputName[100], changeStr[100];
	...
	while (1) {
		...
		if (strcmp(inputName, "끝") == 0)
			break;

		for (i = 0; i < profileCnt; i++) {
			if (strcmp(ary[i].getName().getStr(), inputName) == 0)
				break;
		}
		...
		cout << "    -> 수정 전 Profile : ";
		cout << ary[i].getName().getStr() << " / ";
		cout << ary[i].getAddr().getStr() << " / ";
		cout << ary[i].getAge() << " / ";
		...
		case 1:
			cout << "  * 바꿀 이름을 입력 하세요 : ";
			cin >> changeStr;
			change.setStr(changeStr);
			change.setLen(strlen(changeStr));
		...
		case 2:
			cout << "  * 바꿀 주소를 입력 하세요 : ";
			cin.getline(changeStr, sizeof(changeStr));
			change.setStr(changeStr);
			change.setLen(strlen(changeStr));
			ary[i].setAddr(change);
			break;
		case 3:
			cout << "  * 바꿀 나이를 입력 하세요 : ";
			cin >> ageChange;
